...
const AgentJourneyPage: React.FC = () => {
  ...
  // Function to generate flow from form data
  const generateFlowFromFormData = useCallback(() => {
    if (nodes.length > 0) {
      if (!window.confirm('This will replace your current flow nodes. Continue?')) {
        return;
      }
    }
    ...
    try {
      ...
      // Create nodes based on form data
      const newNodes = [];
      ...
      const startNode = {
        ...
        data: {
          label: 'Start',
          content: `Start of ${formState.agentName || 'Agent'} journey`,
          openNodeEditor
        }
      };
      ...
      if (formState.inputInterpretation) {
        const inputNode = {
          ...
          data: {
            ...
            content: formState.inputInterpretation,
            openNodeEditor
          }
        };
      ...
      if (formState.guardrails) {
        const guardrailNode = {
          id: `node_guardrail_${Date.now()}`,
          type: 'guardrail',
          position: { x: currentX, y: currentY },
          data: {
            label: 'Guardrails',
            content: formState.guardrails,
            openNodeEditor
          }
        };
      ...
      // Decision point
      const decisionNode = {
        id: `node_decision_${Date.now()}`,
        type: 'decision',
        position: { x: currentX, y: currentY },
        data: {
          label: 'Decision Point',
          content: 'Route based on user intent',
          openNodeEditor
        }
      };
      newNodes.push(decisionNode);
      
      // Backend systems nodes - placed below decision point
      if (formState.backendSystems && formState.backendSystems.length > 0) {
        const systemsStartX = currentX;
        let systemY = currentY + gridSpacing;
        
        formState.backendSystems.forEach((system, index) => {
          const systemNode = {
            id: `node_system_${Date.now()}_${index}`,
            type: 'system',
            position: { x: systemsStartX, y: systemY },
            data: {
              label: String(system),
              content: `Integration with ${String(system)}`,
              openNodeEditor
            }
          };
          newNodes.push(systemNode);
          systemY += gridSpacing / 2;
        });
      }
      ...
      if (formState.contextManagement) {
        const contextNode = {
          id: `node_agent_context_${Date.now()}`,
          type: 'agent',
          position: { x: currentX, y: currentY },
          data: {
            label: 'Context Management',
            content: formState.contextManagement,
            openNodeEditor
          }
        };
      ...
      // Add escalation node if rules are defined
      if (formState.escalationRules) {
        const escalationNode = {
          id: `node_escalation_${Date.now()}`,
          type: 'escalation',
          position: { x: currentX, y: currentY - gridSpacing / 2 },
          data: {
            label: 'Escalation',
            content: formState.escalationRules,
            openNodeEditor
          }
        };
      ...
      // Error node if monitoring is defined
      if (formState.errorMonitoring) {
        const errorNode = {
          ...
          data: {
            ...
            content: formState.errorMonitoring,
            openNodeEditor
          }
        };
      ...
      // Connect the main flow nodes
      for (let i = 0; i < newNodes.length - 1; i++) {
        // Skip connecting to Note nodes (they're documentation)
        if (newNodes[i + 1].type === 'note') {
          continue;
        }
        
        // Skip connecting decision node to next node if it's systems or special nodes
        if (newNodes[i].type === 'decision' && 
            (newNodes[i + 1].type === 'system' || 
             newNodes[i + 1].type === 'escalation')) {
          continue;
        }
        
        // Skip connecting system nodes to the main flow
        if (newNodes[i].type === 'system') {
          continue;
        }
        
        newEdges.push({
          id: `edge_${i}_${i+1}_${Date.now()}`,
      ...
      // Add connections from decision to systems
      const decisionNodeIndex = newNodes.findIndex(node => node.type === 'decision');
      if (decisionNodeIndex !== -1) {
        const systemNodes = newNodes.filter(node => node.type === 'system');
        systemNodes.forEach((systemNode, index) => {
          newEdges.push({
            id: `edge_decision_system_${index}_${Date.now()}`,
            source: newNodes[decisionNodeIndex].id,
          ...
          // Connect system nodes back to context management if it exists
          const contextNodeIndex = newNodes.findIndex(node => 
            node.type === 'agent' && 
            node.data.label === 'Context Management'
          );
          
          if (contextNodeIndex !== -1) {
            newEdges.push({
              id: `edge_system_context_${index}_${Date.now()}`,
              source: systemNode.id,
              target: newNodes[contextNodeIndex].id,
              type: 'smoothstep',
              animated: true,
              markerEnd: {
                type: MarkerType.ArrowClosed,
      ...
      toast({
        title: "Success",
        description: "Flow generated successfully from form data!"
      });
      
    } catch (error) {
      console.error('Error generating flow from form data:', error);
      toast({
        title: "Error",
        description: `Failed to generate flow: ${(error as Error).message}`,
        variant: "destructive"
      });
    }
  }, [formState, nodes.length, setNodes, setEdges, isEditing, startEditing, toast, openNodeEditor]);
